...
export default function StackedPositiveNegativeBarChart(props) {
    ...
    function generateBars() {
        var bars = dataSource.bars.map(function (bar, index) {
            ...
            return (React.createElement(Bar, { key: index, barSize: barSize ? barSize : defaultValues.barSize, dataKey: title, fill: color ? color : defaultValues.color, stackId: stackId ? stackId : defaultValues.stackId }));
        });
        return bars;
    }
    return (React.createElement(ResponsiveContainer, { width: width ? width : defaultValues.width, height: height ? height : defaultValues.height },
        React.createElement(BarChart, { data: dataSource.data, stackOffset: "sign" },
